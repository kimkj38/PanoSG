...
def _draw_and_save_output_images(img_detections, imgs, img_size, output_path, classes):
    ...
    for (image_path, detections) in zip(imgs, img_detections):
        ...
        image_id = image_path[-14:-4]
        
        _draw_and_save_output_image(
            image_path, detections, img_size, output_path, classes, image_id)


def _draw_and_save_output_image(image_path, detections, img_size, output_path, classes, image_id):
    ...
    for i, (x1, y1, x2, y2, conf, cls_pred) in enumerate(detections):
        ...
        obj['label'] = int(cls_pred.item())
        obj['bbox'] = [int(x1.item()),int(y1.item()),int(x2.item()),int(y2.item())]

        obj_index = "obj_{}".format(i)
        img[obj_index] = obj
